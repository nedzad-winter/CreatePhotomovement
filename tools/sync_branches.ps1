...
try {
    ...
    foreach ($Branch in $AllBranches) {
        ...
        # Checkout target branch (suppress all output, check exit code)
        ...
        # Merge current branch (suppress all output, check exit code)
        $MergeOutput = git merge $CurrentBranch --no-edit 2>&1
        if ($LASTEXITCODE -ne 0) {
